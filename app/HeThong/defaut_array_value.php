...
function getTaoDuThaoKTCumKhoi(&$model, $maduthao = null)
{
    if ($model->thongtinquyetdinh == '') {
        ...
        //Hộ gia đình        
        $m_hogiadinh = App\Model\NghiepVu\CumKhoiThiDua\dshosotdktcumkhoi_hogiadinh::where('mahosotdkt', $model->mahosotdkt)
            ->where('ketqua', '1')->orderby('stt')->get();
        if ($m_hogiadinh->count() > 0) {
            $s_hgd = '';
            $i = 1;
...
function getTaoQuyetDinhKTKhenCao(&$model, $maduthao)
{
    if ($model->thongtinquyetdinh == '') {
        ...
        $m_canhan = App\Model\NghiepVu\KhenCao\dshosokhencao_canhan::where('mahosotdkt', $model->mahosotdkt)
            ->where('ketqua', '1')->orderby('stt')->get();
        //dd($m_canhan);
        if ($m_canhan->count() > 0) {
            $s_canhan = '';
        ...
        //Tập thể
        $m_tapthe = App\Model\NghiepVu\KhenCao\dshosodenghikhencao_tapthe::where('mahosotdkt', $model->mahosotdkt)
            ->where('ketqua', '1')->orderby('stt')->get();
        if ($m_tapthe->count() > 0) {
            $s_tapthe = '';
            $i = 1;
        ...
        //Hộ gia đình        
        $m_hogiadinh = App\Model\NghiepVu\KhenCao\dshosokhencao_hogiadinh::where('mahosotdkt', $model->mahosotdkt)
            ->where('ketqua', '1')->orderby('stt')->get();
        if ($m_hogiadinh->count() > 0) {
            $s_hgd = '';
            $i = 1;
