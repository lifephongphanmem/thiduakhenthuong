...
class APIdungchungController extends Controller
{
    ...
    public function convertCaNhan($hoso, $danhsachdonvi)
    {
        $tailieu = dshosothiduakhenthuong_tailieu::where('mahosotdkt', $hoso->mahosotdkt)->get();
        $a_kq = [
            'MaHoSo' => $hoso->mahosotdkt,
            'MaCCVC' => $hoso->macongchucvienchuc,
            'TenCanBo' => $hoso->tendoituong,
            'NgaySinh' =>  $hoso->ngaysinh,
            'GioiTinh' =>  $hoso->gioitinh,
            'ChucVu' =>  $hoso->chucvu,
            'DiaChi' =>   $hoso->diachi,
            'TenCoQuan' =>  $hoso->tencoquan,
            'TenPhongBan' =>  $hoso->tenphongban,
            'PhanLoaiCanBo' =>  $hoso->maphanloaicanbo,
            'LoaiHinhKhenThuong' => $hoso->mahinhthuckt,
            'NoiDungKhenThuong' => $hoso->noidung,
            'SoQuyetDinh' => $hoso->soqd,
            'NgayQuyetDinh' => $hoso->ngayqd,
    ...
    public function taoAPI($domain, $linkChucNang, $machung, $madonvi, $mahoso)
    {
        return $domain . $linkChucNang . '?_token=' . base64_encode($machung . ':' . $madonvi . ':' . $mahoso);
    }
}
