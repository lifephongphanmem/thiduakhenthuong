...
@section('content')
    ...
    <table id="data_render" cellspacing="0" cellpadding="0" border="1"
        ...
        $i = 0;
        $i_diaban = 0;
        $diaban = a_getelement($a_diaban, ['capdo' => 'T']);
        ?>
        @foreach ($diaban as $v_diaban)
            ...
            <!-- Dải các địa bàn trực thuộc trong địa bàn -->
            <?php
            $diaban_capduoi_h = a_getelement($a_diaban, ['madiabanQL' => $v_diaban['madiaban']]); //Đơn vị cấp Huyện
            ...
            @if (count($diaban_capduoi_h) > 0 )
                @foreach ($diaban_capduoi_h as $diaban_cd)
                    <?php
                    //Đơn vị cấp Huyện
                    $donvi_h = $model->where('madiaban', (string) $diaban_cd['madiaban']);
                    $k_h = 0;
                    //Đơn vị cấp Xã
