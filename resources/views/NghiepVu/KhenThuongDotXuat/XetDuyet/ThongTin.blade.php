    ...
    <script>
        jQuery(document).ready(function() {
            ...
            $('#madonvi').change(function() {
                window.location.href = '/KhenThuongDotXuat/XetDuyet/ThongTin?madonvi=' + $(
                        '#madonvi').val() +
...
@section('content')
    ...
    <div class="card card-custom wave wave-animate-slow wave-info" style="min-height: 600px">
        <div class="card-header flex-wrap border-1 pt-5 pb-0">
            <div class="card-title">
                <h3 class="card-label text-uppercase">Danh sách hồ sơ trình khen thưởng đột xuất</h3>
            </div>
            <div class="card-toolbar">
                @if (chkPhanQuyen('xdhosokhenthuongdotxuat', 'thaydoi'))
                                ...
                                <td style="text-align: center">
                                    ...
                                    @if (in_array($tt->trangthai_hoso, ['CD']))
                                        <button title="Trả lại hồ sơ" type="button"
                                            onclick="confirmTraLai('{{ $tt->mahosotdkt }}', '{{ $inputs['madonvi'] }}', '/KhenThuongDotXuat/XetDuyet/TraLai')"
                                        ...
                                        <button title="Nhận hồ sơ đăng ký" type="button"
                                            onclick="confirmNhan('{{ $tt->mahosotdkt }}','/KhenThuongDotXuat/XetDuyet/NhanHoSo','{{ $inputs['madonvi'] }}')"
                                    ...
                                    @if (in_array($tt->trangthai_hoso, ['CNXKT']))
                                        <button title="Chuyển hồ sơ đăng ký" type="button"
                                            onclick="confirmChuyen('{{ $tt->mahosotdkt }}','/KhenThuongDotXuat/XetDuyet/ChuyenHoSo')"
